...
#btnCerrar, #btnAceptar, #btnVolver, #btnOk { 
...
<main class="main" style="text-align: center;">
  ...
  <form class="form-signin" id="formNuevoUsuario" method="POST">
    <h1 class="h3 mb-3 fw-normal"><b>Registro nuevo usuario</b></h1>
    ...
    <!--<div class="form-floating">
      <input type="date" class="form-control" id="txtFechaNacimiento" placeholder="Fecha de nacimiento" name="txtFechaNacimiento">
    ...
    <button class="btn btn-primary w-100 py-2" id="btnCrearUsuario">Crear nueva cuenta</button>
  </form>
    ...
    <script>
      $(document).ready(function(){
       $("#btnCrearUsuario").click(function(event){
         ...
         $.ajax({
           type: "POST",
           url: "scripts/crear-usuario.php", 
           data: $("#formNuevoUsuario").serialize(),
           success: function(response){
            if (response == "empty") {
              $("#campoVacio").css("display", "block");

            } else if (response == "usuarioExistente") {
              $("#usuarioExistente").css("display", "block");

            } else if  (response == "registroCorrecto") {
              $("#cuentaCreada").css("display", "block");

            } else {
              $("#errorCrearCuenta").css("display", "block");
            }
          } 
         });
         return false;
        ...
        $(".close, #btnVolver, #btnAceptar, #btnCerrar, #btnOk").click(function(){
        $("#campoVacio, #cuentaCreada, #errorCrearCuenta, #usuarioExistente").css("display", "none");
        });
     });
    </script>
    ...
    <script>
          $(document).ready(function(){
            $("#btnAceptar").click(function(e){
              e.preventDefault();
                $("#contenedorRegistro").load("pagina-inicio.php")
                });
            });
        </script>
...
<div id="campoVacio" class="modal">
  <div class="modal-content">
    <h1 class="h3 mb-3 fw-normal">Campos vacíos</h1>
    <p>Por favor, complete todos los campos</p>
    <button id="btnVolver">Volver</button>
  </div>
</div>

<div id="usuarioExistente" class="modal">
  <div class="modal-content">
    <h1 class="h3 mb-3 fw-normal">Usuario existente</h1>
    <p>Por favor, introduzca un nombre de usuario válido</p>
    <button id="btnOk">Aceptar</button>
  </div>
</div>

<div id="cuentaCreada" class="modal">
  <div class="modal-content">
    <h1 class="h3 mb-3 fw-normal">¡Felicidades!</h1>
    <p>Tu cuenta se ha creado con éxito. Ahora ya puedes iniciar sesión</p>
    <button id="btnAceptar">Aceptar</button>
  </div>
</div>

<div id="errorCrearCuenta" class="modal">
  <div class="modal-content">
